...
module.exports = {
  overrideWebpackConfig: ({
    ...
    webpackConfig.entry = entry || path.resolve(entry || 'src/index.js')
    ...
    if (reactPackagesAsExternal !== false) externals = [...externals, 'react', 'react-dom']

    if (orgPackagesAsExternal === true) externals = [...externals, new RegExp(`^@${orgName}/`)]
  ...
  overrideCracoConfig: ({ cracoConfig, pluginOptions: { orgName, projectName, rootDirectoryLevel } }) => {
    ...
    cracoConfig.devServer.sockHost = 'localhost'
    cracoConfig.devServer.sockPort = process.env.PORT
